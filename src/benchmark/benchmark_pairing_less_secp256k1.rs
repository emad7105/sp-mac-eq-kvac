use crate::benchmark::benchmark_utils_secp256k1::*;
use crate::protocols::kvac_pairing_less_secp256k1::KvacPLSecp256k1;
use std::time::Instant;

const ITERATIONS: usize = 50;

pub fn start_benchmarking() {
    println!("Pairingless (curve: secp256k1) benchmark...\n");
    // S: attributes
...
pub fn benchmark_issue_cred(S_sizes: &[usize], D_sizes: &[usize]) {
    ...
    for (s_size, d_size) in S_sizes.iter().zip(D_sizes.iter()) {
        ...
        for _ in 0..ITERATIONS {
            ...
            pre_cred_size = KvacPLSecp256k1::size_of_pre_cred(&pre_cred);
...
pub fn benchmark_obtain_cred(S_sizes: &[usize], D_sizes: &[usize]) {
    ...
    for (s_size, d_size) in S_sizes.iter().zip(D_sizes.iter()) {
        ...
        for _ in 0..ITERATIONS {
            ...
            cred_size = KvacPLSecp256k1::size_of_credential(&cred);
...
pub fn benchmark_show_cred(S_sizes: &[usize], D_sizes: &[usize]) {
    ...
    for (s_size, d_size) in S_sizes.iter().zip(D_sizes.iter()) {
        ...
        for _ in 0..ITERATIONS {
            ...
            show_size = KvacPLSecp256k1::size_of_show(&show);
