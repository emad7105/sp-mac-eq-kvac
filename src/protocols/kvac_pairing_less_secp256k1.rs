...
use ark_secp256k1::{Fr as ScalarField, Projective as G, FrConfig, Fr, Projective};
use ark_ff::{BigInteger, Field, Fp256, MontBackend, PrimeField};
use ark_serialize::CanonicalSerialize;
use ark_std::{UniformRand, Zero};
use sha2::Sha256;
use crate::protocols::polynomial_utils_secp256k1;
use sha2::Digest;


pub struct KvacPLSecp256k1 {}
...
impl KvacPLSecp256k1 {
    ...
    pub fn issue_cred(pp: &PublicParams, S: &[ScalarField], isk: &SecretKeys) -> PreCredential {
        ...
        // 3. C <- y * f_S(v)G
        let f_S_evaluated_at_v = polynomial_utils_secp256k1::evaluate_f_at_v_without_interpolation(&S, &isk.v);
        // calculate f_S = (x-s1)*(x-s2)...(x-sn) the vanishing polynomial
        // let f_S = polynomial_utils_secp256k1::construct_f(S);
        // f_S(v)
        ...
        let pok = KvacPLSecp256k1::pok(&witness, &C, &pp.R, &pp.X, &Yj, &S, &tau, &pp.V);
    ...
    pub fn obtain_cred(pp: &PublicParams, pre_cred: &PreCredential, S: &[ScalarField]) -> Credential {
        // C <-- y * f_S(v) G
        let f = polynomial_utils_secp256k1::construct_f(&S);
        let C_recalculated = polynomial_utils_secp256k1::evaluate_f_at_secret_point(&f.coeffs, &pre_cred.Yj);

        // Check PoK
        let result = KvacPLSecp256k1::pok_verify(&pre_cred.pok, &pp.V, &C_recalculated, &pp.R, &pp.X, &pre_cred.Yj, &S, &pre_cred.tau);
        assert!(result);
    ...
    pub fn show_cred(pp: &PublicParams, cred: &Credential, S: &[ScalarField], D: &[ScalarField]) -> Show {
        ...
        // S\D
        let S_without_D = polynomial_utils_secp256k1::difference(S, D);
        // f_{S\D}
        let f = polynomial_utils_secp256k1::construct_f(&S_without_D);
        let yf = polynomial_utils_secp256k1::evaluate_f_at_secret_point(&f.coeffs, &cred.Yj);
    ...
    pub fn verify(pp: &PublicParams, show: &Show, D: &[ScalarField], isk: &SecretKeys ) -> bool {
        // xWf_D(v) == tau_prime
        // f_D(x)
        // let f_D = polynomial_utils_secp256k1::construct_f(D);
        // f_D(v)
        // let f_D_evaluated_at_v = f_D.evaluate(&isk.v);
        let f_D_evaluated_at_v = polynomial_utils_secp256k1::evaluate_f_at_v_without_interpolation(&D, &isk.v);
        // x.f_D(v)
    ...
    pub fn pok (witness: &Witness, C: &G, R: &G, X:&G, Yi: &[G], S: &[ScalarField], tau: &G, V: &G) -> PoK{
        ...
        let c = KvacPLSecp256k1::hash_to_fr(&hash_input_points, &S);
    ...
    pub fn pok_verify(pok: &PoK, V: &G, C: &G, R: &G, X: &G, Yj: &[G], S: &[ScalarField], tau: &G) -> bool {
        ...
        let c_verifier = KvacPLSecp256k1::hash_to_fr(&hash_input_points, &S);

        pok.c == c_verifier
    }

    pub fn hash_to_fr(points: &[Projective], scalar_fields: &[ScalarField]) -> Fr {
...
#[cfg(test)]
mod kvac_pairing_less_secp256k1_tests {
    use std::ops::Mul;
    ...
    use crate::protocols::kvac_pairing_less_secp256k1::{KvacPLSecp256k1, ScalarField};
    use crate::protocols::kvac_pairing_less_secp256k1::G;
    use crate::protocols::polynomial_utils_secp256k1;


    #[test]
    pub fn test_secp256k1_operations() {
        let mut rng = ark_std::rand::thread_rng();
        let r = ScalarField::rand(&mut rng);
    ...
    fn test_kvac_full_flow_test() {
        // 0. key gens
        let isk = KvacPLSecp256k1::gen_isk();
        let pp = KvacPLSecp256k1::gen_public_params(&isk);
        ...
        let pre_cred = KvacPLSecp256k1::issue_cred(&pp, &S, &isk);

        // 2. KVAC.obtain_cred(pp, PreCred, S, ipar)
        let cred = KvacPLSecp256k1::obtain_cred(&pp, &pre_cred, &S);

        // 3. KVAC.show_cred(pp, Cred, S, D)
        let show = KvacPLSecp256k1::show_cred(&pp, &cred, &S, &D);

        // 4. KVAC.verify(pp, Show, D, isk)
        let result = KvacPLSecp256k1::verify(&pp, &show, &D, &isk);
    ...
    fn test_construct_f() {
        ...
        let f1 = polynomial_utils_secp256k1::construct_f(&s);
        let f2 = polynomial_utils_secp256k1::construct_f(&s_permuted);
    ...
    fn test_evaluate_f_at_v_without_interpolation() {
        ...
        let fv1 = polynomial_utils_secp256k1::evaluate_f_at_v_without_interpolation(&s, &v);
        let fv2 = polynomial_utils_secp256k1::evaluate_f_at_v_without_interpolation(&s_permuted, &v);
    ...
    #[test]
    fn test_key_gens() {
        let keys = KvacPLSecp256k1::gen_isk();
        let public_params = KvacPLSecp256k1::gen_public_params(&keys);
    }

    #[test]
    fn test_evaluations() {
        let secret_keys = KvacPLSecp256k1::gen_isk();
        let public_params = KvacPLSecp256k1::gen_public_params(&secret_keys);

        let S = prepare_set_S(20);

        let pre_cred = KvacPLSecp256k1::issue_cred(&public_params, &S, &secret_keys);
        ...
        let C = polynomial_utils_secp256k1::evaluate_f_at_secret_point(&f_S.coeffs, &pre_cred.Yj);
    ...
    fn test_Kvac_flow() {
        ...
        let pre_cred = KvacPLSecp256k1::issue_cred(&public_params, &S, &secret_keys);
        let credential = KvacPLSecp256k1::obtain_cred(&public_params, &pre_cred, &S);
    }
